...
export default function Index() {
  ...
  const handleEvaluate = () => {
    ...
    setEvaluating(true);
    axios.get(`${process.env.NEXT_PUBLIC_SERVER_HOST}/svm/evaluate?query=${evaluateText}`, {
      headers: {
        'Authorization': `${token_type} ${access_token}`
      }
    }).then(({data}) => {
  ...
  return (
    <div className={styles.lexiconLayout}>
      ...
      <Box className={styles.left} sx={{paddingTop: 2}}>
        <h1 style={{paddingLeft: 20, margin: '20px 0 0'}}>Support Vector Machines (SVM)</h1>
        <p style={{padding: '0 20px', marginBottom: 0}}>En esta página se muestran los resultados obtenidos generando un modelo SVM usando scikit-learn.</p>
